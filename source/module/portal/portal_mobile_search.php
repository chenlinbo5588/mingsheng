...
if(!submitcheck('searchsubmit', 1)) {
    ...
    */
	...
	if(!empty($searchid)) {
		...
		$multipage = multi($index['num'], $_G['tpp'], $page, "m.php?mod=mobile_search&mobile=yes&searchid=$searchid&orderby=$orderby&ascdesc=$ascdesc&searchsubmit=yes");

		$url_forward = 'm.php?mod=mobile_search&mobile=yes&'.$_SERVER['QUERY_STRING'];
        ...
        //print_r($threadlist);
		//include template('search/forum');
        include_once template('mobile_search');
	} else {
		...
		dheader("location: m.php?mod=mobile_search&mobile=yes&searchid=$searchid&orderby=$orderby&ascdesc=$ascdesc&searchsubmit=yes&kw=".urlencode($keyword));
