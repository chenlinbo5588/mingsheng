...
if(!submitcheck('searchsubmit', 1)) {
	...
	dheader("location: m.php?mobile=yes");
	...
	if(!empty($searchid)) {
		...
		$multipage = multi($index['num'], $_G['tpp'], $page, "m.php?mod=mobile_psearch&mobile=yes&searchid=$searchid&orderby=$orderby&ascdesc=$ascdesc&searchsubmit=yes");

		$url_forward = 'm.php?mod=mobile_psearch&mobile=yes&'.$_SERVER['QUERY_STRING'];

		include template('mobile_psearch');
		...
		foreach(C::t('common_searchindex')->fetch_all_search($_G['setting']['search']['portal']['searchctrl'], $_G['clientip'], $_G['uid'], $_G['timestamp'], $searchstring, $srchmod) as $index) {
			if($index['indexvalid'] && $index['dateline'] > $searchindex['dateline']) {
				...
				showmessage('search_ctrl', 'm.php?mod=mobile_psearch&mobile=yes', array('searchctrl' => $_G['setting']['search']['portal']['searchctrl']));
			}
		}
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt) {
				dheader('Location: m.php?mod=mobile_psearch&mobile=yes');
			}
			...
			if($_G['adminid'] != '1' && $_G['setting']['search']['portal']['maxspm']) {
				if(C::t('common_searchindex')->count_by_dateline($_G['timestamp'], $srchmod) >= $_G['setting']['search']['portal']['maxspm']) {
					showmessage('search_toomany', 'm.php?mod=mobile_psearch&mobile=yes', array('maxspm' => $_G['setting']['search']['portal']['maxspm']));
				}
			}
		...
		dheader("location: m.php?mod=mobile_psearch&mobile=yes&searchid=$searchid&searchsubmit=yes&kw=".urlencode($keyword));
