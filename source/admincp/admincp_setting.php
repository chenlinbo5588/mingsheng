...
EOT;
    ...
    } elseif($operation == 'holiday') {
        $yearList = range(2014,date("Y") + 1);
        $showYear = $_GET['year'] ? $_GET['year'] : date("Y");
        
        echo '<label>请选择设置的年份:<select name="year" onchange="location.href=\'admin.php?action=setting&operation=holiday&year=\' + this.value">';
        foreach($yearList as $v){
            echo '<option value="'.$v.'" ' . ($v == $showYear ? ' selected' : '') . '>'.$v.'</option>';
        }
        echo '</select></label>';
        
        print <<<EOF
        <input type="button" class="btn" name="checkedWeekend" value="勾选双休日"/>
        &nbsp;<input type="button" class="btn" name="uncheckedWeekend" value="取消勾选双休日"/>
        &nbsp;<input type="button" class="btn" name="checkedSaturday" value="勾选星期六"/>
        &nbsp;<input type="button" class="btn" name="uncheckedSaturday" value="取消勾选星期六"/>
        &nbsp;<input type="button" class="btn" name="checkedSunday"  value="勾选星期天"/>
        &nbsp;<input type="button" class="btn" name="uncheckedSunday"  value="取消勾选星期天"/>
        <script src="static/js/jquery.js" type="text/javascript"></script>
        <script>
         var jq = jQuery.noConflict();
         
         jq(function(){
                jq("input[name=checkedWeekend]").bind("click",function(){
                    jq(".saturday input[type=checkbox],.sunday input[type=checkbox]").prop("checked",true);
                });
                jq("input[name=uncheckedWeekend]").bind("click",function(){
                    jq(".saturday input[type=checkbox],.sunday input[type=checkbox]").prop("checked",false);
                });
                jq("input[name=checkedSaturday]").bind("click",function(){
                    jq(".saturday input[type=checkbox]").prop("checked",true);
                });
                jq("input[name=uncheckedSaturday]").bind("click",function(){
                    jq(".saturday input[type=checkbox]").prop("checked",false);
                });
                jq("input[name=checkedSunday]").bind("click",function(){
                    jq(".sunday input[type=checkbox]").prop("checked",true);
                });
                jq("input[name=uncheckedSunday]").bind("click",function(){
                    jq(".sunday input[type=checkbox]").prop("checked",false);
                });
         });
        </script>
EOF;
        ...
        $cal = new Calendar($prefs);
        
        
        $holidayList = C::t('common_holiday')->fetch_by_year($showYear);
        $hdList = array();
        foreach($holidayList as $v){
            $hdList[$v['date_key']] = "checked";
        }
        
        $months = range(1,12);
        echo '<div class="holiday_calendar">';
        foreach($months as $val){
            echo '<div class="cal_item">'.$cal->generate($showYear,$val,$hdList).'</div>';
        }
        echo '</div>';
    ...
    if($operation == 'holiday') {
        ...
        if(!empty($_POST['year']) && !empty($_POST['sd'])){
	...
	cpmsg('setting_update_succeed', 'action=setting&operation='.$operation.(!empty($_GET['anchor']) ? '&anchor='.$_GET['anchor'] : '').(!empty($from) ? '&from='.$from : '').(!empty($showYear) ? '&year='.$showYear : ''), 'succeed');
}
...
class Calendar {
	...
	function generate($year = '', $month = '', $data = array())
	{
        ...
        $day_keys = $this->get_day_key();
        
		for ($i = 0; $i < 7; $i ++)
		{
			$out .= str_replace(array('{week_day_class}' ,  '{week_day}'), array($day_keys[($start_day + $i) %7], $day_names[($start_day + $i) %7]), $this->temp['week_day_cell']);
		}
		...
		while ($day <= $total_days)
		{
			...
			for ($i = 0; $i < 7; $i++)
			{
				$out .= ($is_current_month == TRUE AND $day == $cur_day) ? str_replace('{week_day_class}',($day_keys[($start_day + $i) %7]), $this->temp['cal_cell_start_today']) : str_replace('{week_day_class}',($day_keys[($start_day + $i) %7]),$this->temp['cal_cell_start']);
	...
	function get_month_name($month)
	{
		...
		if ($this->line($month) === FALSE)
		{
			return ucfirst(str_replace('cal_', '', $month));
		}

		return $this->line($month);
	}

	// --------------------------------------------------------------------
    
    function get_day_key($day_type = ''){
	...
	function get_day_names($day_type = '')
	{
		...
		{
			$days[] = ($this->line('cal_'.$val) === FALSE) ? ucfirst($val) : $this->line('cal_'.$val);
		}
	...
	{
		return  array (
						'table_open'				=> '<table class="calendar" border="0" cellpadding="4" cellspacing="0">',
						'heading_row_start'			=> '<tr class="cal_header">',
						'heading_previous_cell'		=> '<th><a href="{previous_url}">&lt;&lt;</a></th>',
						'heading_title_cell'		=> '<th colspan="{colspan}">{heading}</th>',
						'heading_next_cell'			=> '<th><a href="{next_url}">&gt;&gt;</a></th>',
						'heading_row_end'			=> '</tr>',
						'week_row_start'			=> '<tr>',
						'week_day_cell'				=> '<td class="{week_day_class}">{week_day}</td>',
						'week_row_end'				=> '</tr>',
						'cal_row_start'				=> '<tr>',
						'cal_cell_start'			=> '<td class="{week_day_class}"><div class="day_wrap">',
						'cal_cell_start_today'		=> '<td class="{week_day_class}"><div class="day_wrap">',
						'cal_cell_content'			=> '<div class="rest">放假</div><label><input type="checkbox" name="sd[]" value="{date-key}" {content}> {day}</label>',
						'cal_cell_content_today'	=> '<div class="rest">放假</div><label><input type="checkbox" name="sd[]" value="{date-key}" {content}> <strong>{day}</strong></label>',
						'cal_cell_no_content'		=> '<div class="work">上班</div><label><input type="checkbox" name="sd[]" value="{date-key}" {content}> {day}</label>',
						'cal_cell_no_content_today'	=> '<div class="work">上班</div><label><input type="checkbox" name="sd[]" value="{date-key}" {content}><strong>{day}</strong></label>',
						'cal_cell_blank'			=> '&nbsp;',
						'cal_cell_end'				=> '</div></td>',
						'cal_cell_end_today'		=> '</div></td>',
						'cal_row_end'				=> '</tr>',
						'table_close'				=> '</table>'
					);
	}
